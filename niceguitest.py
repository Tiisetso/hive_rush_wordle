...
with ui.row().classes('w-full justify-center').style('margin-top: 5vh'):
    for i in range(5):
        cell = (ui.input()
              .props('maxlength=1 outlined input-class="font-mono h-full w-full rounded-none"')
    ...
    ui.button('GO', on_click=lambda _: capture_state()).props('unelevated').style('width: 55px; height: 55px; font-size: 16px;')
...
with ui.card().style('margin: 2vh auto; width: 400px; max-height: 200px; overflow-y: auto;').props('flat'):
    for w in words:
        ui.label(w)
